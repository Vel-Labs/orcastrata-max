---
name: codexmax-orchestrate
description: "Primary entry for automatic-loader smoke checks and governed Codex outcomes; returns an honest host-observable receipt."
---

# Orcastrata Max Orchestrate

Use this skill as the normal natural-language front door. Preserve the
operator's original request. One visible Parent owns scope, integration,
verification, and final acceptance.

## First Use

Use the current installed skill and package provenance. Read the request and
nearest `AGENTS.md`. Do not scan historical plugin caches, reports, goals,
...
When the operator starts with `Use model X through tool Y`, with or without one
...
1. Handle bounded work in the Parent when delegation overhead exceeds its
   value.
2. Delegate one bounded task only when an independent lane can add material
   confidence, challenge a decision, or shorten the critical path.
...
material defect. Run the full repository suite once only at a frozen package
boundary.
...
```
<!-- codexmax-automatic-loader-receipt:end -->
